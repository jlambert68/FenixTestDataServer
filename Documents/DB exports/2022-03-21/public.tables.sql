...
create table if not exists testdata_header_filter_selection_types
(
...
create unique index if not exists "testdata.header.selection_types_header_selection_type_uindex"
    on testdata_header_filter_selection_types (header_filter_selection_type);

create table if not exists client_areatype
(
...
create table if not exists testdata_merklehashes
(
    client_uuid            uuid      not null
...
create table if not exists testdata_merkletrees
(
    client_uuid       uuid      not null
    ...
    node_filter_path  varchar   not null,
...
create table if not exists testdata_row_items_current
(
    client_uuid              uuid      not null
...
create table if not exists testdata_row_items_history
(
    client_uuid              uuid      not null
...
comment on table testdata_row_items_history is 'Hold all old test data items';

alter table testdata_row_items_history
    owner to postgres;

create unique index if not exists "client.areatype_client_areatype_id_uindex"
    on client_areatype (client_areatype_id);

create table if not exists "testdata_headerItems_hashes"
(
    client_uuid        uuid      not null
...
create table if not exists testdata_header_items
(
    client_uuid              uuid      not null
...
create table if not exists testdata_header_filtervalues
(
    header_item_hash          varchar   not null
...
create index if not exists "testdata.header.filtervalues_header_item_hash_index"
